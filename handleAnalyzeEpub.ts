...
import { analyzeEpub, Analysis } from './analyzeEpub';
...
export async function handleAnalyzeEpubTrigger(
    ...
    const { publisher_id, material_id, source } = context.params as any;
    const { storage_bucket, storage_path, material_ext } = change.after.val() as MaterialData;

    // Handle audiobooks elsewhere
    if (material_ext !== 'epub') return;

    if (!material_id) throw Error('No material identifier found in metadata');
    if (!publisher_id) throw Error('No publisher identifier found in metadata');
    ...
    try {
        ...
        const analysis = await analyzeEpub(tempFilePath) as Analysis;
        await Promise.all([
        ...
        fs.unlink(tempFilePath, error => error && console.error(`Failed to delete temporary file '${tempFilePath}': ${error}`));

        console.log(`[${handleAnalyzeEpubTrigger.name}] Successfully analyzed material '${material_id}' from publisher '${publisher_id}'`);
        return analysis;
    } catch (error) {
        const message = `[${handleAnalyzeEpubTrigger.name}] Failed to analyze material '${material_id}': ${error}` as string;
        console.log(message);
        throw Error(message);
    }
}
...
export async function handleAnalyzeEpubAPI(data: { material_id: string }) {
    try {
        ...
        const matRegex = /(?<![0-9])[0-9]{13}(?![0-9])/;
        if (!material_id || !matRegex.test(material_id)) return 'Missing or invalid material identifier.';
        ...
        if (material.material_ext !== 'epub') return `Cannot analyze material '${material_id}' because file type is '${material.material_ext}'`;

        // Download EPUB file from Firebase Storage to local temporary directory and get path
        const tempFilePath = await downloadEpubFromStorage(material_id, material.storage_bucket, material.storage_path);
        ...
        console.log(`[${handleAnalyzeEpubAPI.name}] Successfully analyzed material '${material_id}' from publisher '${publisher_id}'`);
        return analysis;
    } catch (error) {
        const message = `[${handleAnalyzeEpubAPI.name}] Failed to analyze material '${data.material_id}': ${error}` as string;
        console.log(message);
        throw Error(message);
    }
}
...
async function getPublisherIdFromFirestore(material_id: string): Promise<string> {
    try {
        ...
        if (material.empty) throw Error(`No material found with ID '${material_id}' in Firestore`);

        const publisher_id = material.docs[0].get('publisher_id');
        return publisher_id;
    } catch (error) {
        throw Error(`Failed to retrieve publisher ID from Firestore for material '${material_id}': ${error}`);
    }
}
 ...
 */
async function getLatestMaterialFromRealtime(material_id: string, publisher_id: string
): Promise<{ source: string, material: MaterialData }> {
    try {
        const materialBySourceSnapshot = await realtime
            .ref(`material-upload-completion/${publisher_id}/sources/${material_id}`)
            .once('value');

        const materialBySource = materialBySourceSnapshot.val() as MaterialBySource;
        if (!materialBySource) throw Error(`No material found from publisher '${publisher_id}' with ID '${material_id}' in Realtime Database.`);
        ...
        if (!latestMaterial) throw Error(`No latest material found for material '${material_id}'`);

        return latestMaterial;
    } catch (error) {
        throw Error(`Failed to retrieve data from Realtime Database for material '${material_id}': ${error}`);
    }
}
...
async function downloadEpubFromStorage(
    ...
    try {
        ...
        if (!exists) throw Error(`Material '${material_id}' not found at '${storage_path}'`);
        ...
        throw Error(`Failed to download file for material '${material_id}': ${error}`);
    }
}
...
async function uploadEpubStatsToFirestore(
    ...
    try {
        ...
        await firestore
            .collection(`/application/application_data/publishers/${publisher_id}/books/${material_id}/source_data/`)
            .doc(newId)
            .set(newData);

    } catch (error) {
        throw Error(`Failed to update Firestore with analysis for material '${material_id}': ${error}`);
    }
}
...
async function uploadEpubStatsToRealtime(
    ...
    try {
        await realtime.ref('/').update({
            ...
            [`material-upload-completion/${publisher_id}/sources/${material_id}/${source}/analysis/update_time`]: new Date().toISOString(),
        });
    } catch (error) {
        throw Error(`Failed to update Realtime Database with analysis for material '${material_id}': ${error}`);
    }
}